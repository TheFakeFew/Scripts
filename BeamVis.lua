...
ArtificialHB.Event:Connect(function()
	...
	local col = math.clamp((loudness*30)*(#visframes/(#visframes*math.random(1,2))), .5, 1)
	...
	for i,v in next, visframes do
		...
		local col = math.clamp((loudness*30)*(i/(#visframes*math.random(1,2))), .5, 1)
		local beam = beams[i-1]
		local beam2 = beams2[i-1]
		v.self.CFrame = v.self.CFrame:Lerp(v.OrigCF*CFrame.new(0,(noise > 0 and noise or -noise)*(loudness*40),0), .1)
		if(beam)then
