		...
		return function(character, t)
			if not character:FindFirstChild("Frozen") then
				...
				for _,v in pairs(freezeparts) do
					...
					a.Velocity = CFrame.Angles(math.rad(math.random(-360,360)),math.rad(math.random(-360,360)),math.rad(math.random(-360,360))).lookVector*25
...
function camshake(direction, intensity, duration)
	if direction:lower() == "inout" then
		Camera.FieldOfView = intensity
		game:GetService('TweenService'):Create(Camera, TweenInfo.new(duration, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {FieldOfView = 70}):Play()
...
addattack(Enum.KeyCode.G, function()
	if timestate == "Slow" then
		...
		game:GetService('TweenService'):Create(Camera, TweenInfo.new(0.5, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out), {FieldOfView = 70}):Play()
	elseif timestate == "None" then
		...
		spawn(function()
			...
			local function bind(obj)
				...
				if obj:IsA("BasePart") and not obj:IsDescendantOf(char) then
					local a = Instance.new("BodyVelocity", obj)
					a.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
					a.Velocity = obj.Velocity/10
					game:GetService("Debris"):AddItem(a, .1)
				end
		...
		Camera.FieldOfView = 120
		game:GetService('TweenService'):Create(Camera, TweenInfo.new(1.8, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out), {FieldOfView = 70}):Play()
	end
end)
local attacks = 0
local lastattack = ""
...
addattack(Enum.KeyCode.Y, function()
	...
	local a = Instance.new("BodyVelocity", rootpart)
	a.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
	a.Velocity = Vector3.new(0,70,0)+rootpart.CFrame.lookVector
	game:GetService("Debris"):AddItem(a, .1)
	local hits = {}
	local p = Instance.new("Part")
	p.Anchored = false
