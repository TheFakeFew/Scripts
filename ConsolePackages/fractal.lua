return {
    func = function(type, iterations)
        ...
        local function drawFractalTree(startPos, length, width, angle, depth, istrunk, last)
            ...
            local trunk = createFrame(UDim2.new(0, math.ceil(width), 0, math.ceil(length)), startPos, Color3.fromHSV(depth/iterations, 1, 1))
            ...
            if(istrunk)then
                ...
                trunk.Position = UDim2.fromScale(2 * (angle > 0 and 1 or -0.75), 0, 0, 0)
                trunk.Parent = last
            end
